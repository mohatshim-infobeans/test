...
/*
function check_file_name_exists($file){ 
    
    $filename = $file['name'];
	$posts_array = get_posts(array('name' => $filename, 'post_type' => 'attachment'));

	if(count($posts_array)>0){
	?>
	<div class="error notice">
    <p>File with this name already exists.</p>
	</div>
	<?php	
		exit;
	}
...
add_filter('wp_handle_upload_prefilter', 'check_file_name_exists');
*/
...
function custom_upload_filter($file) {
    ...
    if ( $image_with !== 1500 || $image_height !== 1000 ) {
...
function check_file_name_exists($file){ 
	...
	$attachments = get_posts(array('post_title' => $filename, 'post_type' => 'attachment'));

    if ($attachments) {
    	$file['error'] = "File name already exists";
	}
...
function check_file_name_exists($file){ 
	global $wpdb;
    $filenm = $file['name'];
    $info = pathinfo($filenm);
	$filename =  basename($filenm,'.'.$info['extension']);
   
  //$attachments = get_posts(array('post_title' => $filename, 'post_type' => 'attachment'));
  //$query = "SELECT COUNT(*) FROM {$wpdb->posts}  WHERE post_title='git_tutorial' AND post_type='attachment'";
    $query = "SELECT COUNT(*) FROM {$wpdb->posts}  WHERE post_title='".$filename."' AND post_type='attachment'";
...
function check_file_name_exists($file){ 
    ...
    $filenm = $file['name'];  //git_tutorial.pdf
    $info = pathinfo($filenm);
	$filename =  basename($filenm,'.'.$info['extension']);	//git_tutorial
   
  //$attachments = get_posts(array('post_title' => $filename, 'post_type' => 'attachment'));
  //$query = "SELECT COUNT(*) FROM {$wpdb->posts}  WHERE post_title='git_tutorial' AND post_type='attachment'";
  //$query = "SELECT COUNT(*) FROM {$wpdb->posts}  WHERE post_title='".$filename."' AND post_type='attachment'";

    $query = "SELECT COUNT(*) FROM {$wpdb->posts}  WHERE post_name='".$filename."' AND post_type='attachment'";
...
function check_file_name_exists($file){ 
    ...
    $filenm = $file['name'];  							   //test.pdf
    $info = pathinfo($filenm);
	$filename =  basename($filenm,'.'.$info['extension']);	//test
   	
   	$args = array('name' => "'".$filename."'", 'post_type' => 'attachment');

    $attachments = get_posts($args);
	
    if ($attachments) {
    	
		$file['error'] = "File name already exists. count=".count($attachments); 
	}
